...
// Check connection
    ...
    if ($conn->connect_error) {
        ...
        if (empty($_POST["new_password"]) && empty($_POST["cfm_password"])) {
        ...
        } elseif (empty($_POST["new_password"]) || empty($_POST["cfm_password"])) {
            //echo "<script>console.log('here')</script>";
            ...
            if ($_POST["new_password"] != $_POST["cfm_password"]) {
                ...
                $errorMsg = "New and confirm passwords do not match. <br>";
                $success = false;
            } else {
...
function process_update($conn, $update_fname, $update_lname, $update_email,
    ...
    if ($newpwd_hashed == null) {
        //echo "<script>console.log('here')</script>";
        $stmt = $conn->prepare("UPDATE Group2.Users SET "
                . "User_fname = (?), User_lname = (?), User_email = (?), User_address = (?), User_postcode = (?), User_phone = (?)"
                . "WHERE User_id = (?)");
        // Bind & execute the query statement:
        $stmt->bind_param("ssssiii", $update_fname, $update_lname, $update_email,
                $update_address, $update_postcode, $update_phoneno, $get_userid);
    } else {
        $stmt = $conn->prepare("UPDATE Group2.Users SET "
